...
if(isset($_POST['submit'])){
    ...
    // Insert found item
    mysqli_query($conn, "INSERT INTO found_items (user_id, title, category, description, image) VALUES ('$user_id', '$title', '$category', '$description', '$image')");
    $success = "Found item reported successfully!";

    // --- Matching Logic ---
    $match_query = mysqli_query($conn, "SELECT * FROM lost_items WHERE title LIKE '%$title%' AND category='$category'");

    if(mysqli_num_rows($match_query) > 0){
        while($match = mysqli_fetch_assoc($match_query)){
            $lost_user_id = $match['user_id'];
            $message = "A found item matches your lost item: ".$match['title'];
            mysqli_query($conn, "INSERT INTO notifications (user_id, message) VALUES ('$lost_user_id', '$message')");

            // Optional: send email to the lost item owner
            // sendEmail($lost_user_email, "Lost Item Match Found", $message);
        }
        $success .= " A matching lost item owner has been notified!";
    }
}
?>
